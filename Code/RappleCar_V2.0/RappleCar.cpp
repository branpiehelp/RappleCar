...
void RappleCar::fwAccelL(int n)
{
  int templ;
  if(ml.get_dir()==FD){
    templ=ml.get_speed()+n;
    if(templ>=0 && templ<255){
      forwardL(templ);
    }
  } 
}
void RappleCar::fwAccelR(int n)
{
  int tempr;
  if(mr.get_dir()==FD){
    tempr=mr.get_speed()+n;
    if(tempr>=0 && tempr<255)
      forwardR(tempr);
  }  
}
void RappleCar::turnRight(int n)
{
